...
/* (0.5 puntos) función necesaria para crear un esquema de cifrado vacío, sin
nodos*/
void crearEsquemaDeCifrado (TCifrado *cf);

/* (3 puntos) función que pone un nodo al final de un esquema de cifrado, si es
posible. Se debe devolver en el último parámetro un valor lógico que sea verdadero
si ha sido posible realizar la operación. No se debe suponer que el valor de
box.sig es válido.*/
void insertarBox (TCifrado * cf, struct TBox box, unsigned char *ok);

/* (1.5 puntos) función que dado un nodo y un valor, devuelve el resultado de
aplicar dicho nodo a dicho valor. Deberás de tener en cuenta si el nodo es una
SumaBox o una XORBox. En el último caso, necesitarás usar operadores lógicos a
nivel de bit, como &, |, ^ o bien ~, así como probablemente usar constantes
numéricas. */
unsigned char aplicarBox (struct TBox box, unsigned char valor);

/* (1.5 puntos) función que toma un esquema de cifrado y un valor, y devuelve el
resultado de aplicar dicho esquema de cifrado a dicho valor, según el método
...
parámetro, de modo que el al final el fichero únicamente contenga dichos datos.
