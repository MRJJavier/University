...
int memoriaDinamica(){
	double *a;
	a = (double *)malloc(5*sizeof(double)); //pido 40 bytes al SO
							 //, y hago el casting al tipo de la variable a
	// equivalente a malloc(40)
	// un array de 5 doubles
	//si ya no necesito la memoria reservada hay que hacer free(a)
	//antes de volver a usar la variable. Si no se produce memory leak.
	printf("a: %u MD: %u\n",&a, a);
	for (int i = 0; i<5; i++){
		*(a+i)=25.0 + (double)i;
...
int devolverMemoria(){
	...
	for (int i=0; i<5; i++){
		//printf("%u ",a[i]);
		//printf("%u ",*(a+i));
		//printf("%u ",a+i); <-- muestra dirección de memoria de las variables
	}
...
int main14(){
	...
	//*********************
	//char a[5] = "Hola";
	//char *a; //esta inicialización no vale
	//arrayParametros1(a);
	//printf("%s",a);
	//*********************
	//int a[5];
