...
// ...
			if(dicVk.isDefinedAt(kvTuple._2())) {
				...
				Set<K> avlset = new AVLSet<>();
				avlset.insert(kvTuple._1());
				dicVk.insert(kvTuple._2(),avlset);
			}
		}
		return dicVk;
	}
...
// Hay que completar aquí  
 		...
 		int cont=1;
 		Set<K> avlset = new AVLSet<K>();
 		for(Tuple2<V,Set<K>> vsetkTuple : dicVk.keysValues()) {
 			if(cont==dicVk.size())
 				avlset=vsetkTuple._2();
 			cont++;
 		}
