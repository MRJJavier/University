...
merge h@(Node x sz lh rh) h'@(Node x' sz' lh' rh')
 | x < x'          = Node x hi a (merge b c)
 | otherwise       = Node x' hi' a' (merge b' c')
 where
   (a,b,c) = pMayor h' lh rh
   (a',b',c') = pMayor h lh' rh'
   hi = 1 + size a + size (merge b c)
   hi' = 1 + size a' + size (merge b' c')
pMayor :: (Ord a) => Heap a -> Heap a -> Heap a -> (Heap a,Heap a,Heap a)
pMayor x y z | size x < size y = pMayor y x z
             | size y < size z = pMayor x z y
             | otherwise = (x,y,z)
----------------------------------------------------------
-- ^^^^^^^^^^^^^^-- SOLO TOCAR ARRIBA ^^^^^^^^^^^ ---------
----------------------------------------------------------
