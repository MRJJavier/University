...
dijkstraAux g vs rs d = dijkstraAux g (v':vs) (delete v' rs) (D.insert v' (p',cs') d)
   where
      ...
      extensiones = [(u,p+w,cs++[u]) | (v,(p,cs)) <- caminos, (u,w) <- successors g v, u `elem` rs]
      --- seleccionar la extensión de menor coste
      (v', p', cs') = minimumBy comparaCaminos extensiones
      comparaCaminos (_, p1, _) (_, p2, _) = compare p1 p2

shortestPaths :: Ord a => WeightedGraph a Int -> a -> [Path a]
shortestPaths g v = [path | (u,(w,path)) <- D.keysAndValues dijkstraResult]
  where
    dijkstraResult = dijkstra g v
