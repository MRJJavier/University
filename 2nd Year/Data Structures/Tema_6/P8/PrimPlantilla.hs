...
primAux g _  [] st = st
primAux g ts rs st = primAux g (y:ts) (delete y rs) (arista:st)
 where
   -- encontrar las aristas de un vértice de ts a otro de rs
   aristas = [ (WE x p y) | (WE x p y) <- edges g, x `elem` ts && y `elem` rs]
   -- de todas las aristas calculadas en el paso anterior encuentra la de menor peso
   arista@(WE x p y) = minimumBy comparaArcos aristas



comparaArcos ::(Ord w) => WeigthedEdge a w -> WeigthedEdge a w -> Ordering
