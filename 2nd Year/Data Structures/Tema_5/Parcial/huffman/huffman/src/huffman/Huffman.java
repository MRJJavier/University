...
public class Huffman {
    ...
    public static Dictionary<Character, List<Integer>> huffmanCode(WLeafTree<Character> ht) {
    	...
    	if(!ht.isLeaf()) {
    		dicClil = prefixWith(0,huffmanCode(ht.leftChild()));
    		dicClir = prefixWith(1,huffmanCode(ht.rightChild()));
    		dicCli=joinDics(dicClil,dicClir);
    	}else {
    ...
    public static String decode(List<Integer> bits, WLeafTree<Character> ht) {
    	...
    	String s=null;
    	while(!bits.isEmpty()) {
    		WLeafTree<Character> copy = ht;
    		while(!copy.isLeaf()) {
    			copy = bits.get(0) == 0 ? copy.leftChild() : copy.rightChild();
    			bits.remove(0);
    		}
    		if(s == null) s="";
    		s = s.concat(copy.elem().toString());
    	}
    	return s;
    }
}
