...
new_dataECB = unpad(decipher_AES.decrypt(ciphertext), BLOCK_SIZE).decode("utf-8", "ignore")
print(new_dataECB)
ha = SHA256.new(new_dataECB.encode("utf-8"))
# Comprobamos el hash
if(ha.digest==hash):
    print (new_dataECB)
...
h = SHA256.new(cadena.encode("utf-8")) # Ya veremos los hash la semana que viene
...
cipherECB = DES.new(K_AT, DES.MODE_ECB)
ciphertextECB = cipherECB.encrypt(pad(M.encode("utf-8"),BLOCK_SIZE))

# A-> T ##### No cambiar este codigo ####
M = None
# A envia: cifrado_K_AT
file_out = open("a_t.bin", "wb")
file_out.write(ciphertextECB)
file_out.close()
cifrado_K_AT = None
# T recibe: msg_K_AT
...
descipherDES = DES.new(K_BT, DES.MODE_ECB)
descifrado_K_BT = unpad(descipherDES.decrypt(msg_K_BT), BLOCK_SIZE_8).decode("utf-8")
print("2-------------------")
