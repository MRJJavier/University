...
class RSA_OBJECT:
    ...
    def __init__(self):
        ...
        # Nota: Para comprobar si un objeto (no) ha sido inicializado, hay que hacer "if self.public_key is None:"
    ...
    def save_PrivateKey(self, file, password):
        ...
        if self.public_key is None:
            raise Exception("RSA_OBJECT not initializated")
        self.private_key = self.private_key.export_key(passphrase=password, pkcs=8, protection="scryptAndAES128-CBC")
    ...
    def load_PrivateKey(self, file, password):
        ...
        cipher_key = open(file,"rb").read()
        self.private_key = RSA.import_key(cipher_key,passphrase=password)
    ...
    def save_PublicKey(self, file):
        ...
        if self.public_key is None:
            raise Exception("RSA_OBJECT not initializated")
        self.public_key=self.public_key.export_key()
        file_out = open(file,"wb")
    ...
    def load_PublicKey(self, file):
        ...
        self.public_key=RSA.import_key(open(file,"rb").read())
    ...
    def cifrar(self, datos):
        ...
        try:
            engineRSA=PKCS1_OAEP.new(self.public_key)
            encriptado = engineRSA.encrypt(datos)
            return encriptado
    ...
    def descifrar(self, cifrado):
        ...
        try:
            engineRSA=PKCS1_OAEP.new(self.private_key)
            datos=engineRSA.decrypt(cifrado)
            return datos
        except (ValueError, KeyError):
            return None
...
# Crear clave RSA y guardar en ficheros la clave privada (protegida) y publica
password = "password"
